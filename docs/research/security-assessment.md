...
Srclight indexes code into a local SQLite database using hybrid search (FTS5 trigram + semantic embeddings). Embeddings are generated by the internal OpenAI-compatible inference server (`text-embedding-gte-multilingual-base`). No local model download is required.

**Findings:**

| ID | Severity | Finding |
|----|----------|---------|
| SL-1 | MEDIUM | If the internal inference server is unreachable, embedding generation fails silently and indexing falls back to keyword-only search. |
| SL-2 | LOW | SQLite database (`.srclight/index.db`) stores embedding vectors that encode semantic meaning of source code. No encryption at rest. |

**Mitigations:**

```bash
# Verify internal inference server is reachable before indexing
curl http://inference.internal/v1/models
```

```yaml
# .srclight/config.yml - point to internal server
embeddings:
...
| Model | Source | Action |
|-------|--------|--------|
| `text-embedding-gte-multilingual-base` | Internal inference server | Verify server is reachable: `curl http://inference.internal/v1/models` |
| clangd | LLVM releases | Download binary from llvm.org, transfer via secure media |

All external downloads must happen on a network-connected staging machine, then transfer to the air-gapped environment via verified removable media.
...
| 3 | Srclight | Internal inference server reachable | `curl http://inference.internal/v1/models` returns model list | [ ] |
| 4 | Srclight | Config points to internal server | `grep base_url .srclight/config.yml` shows internal URL | [ ] |
| 5 | Srclight | Correct model configured | `grep model .srclight/config.yml` shows `text-embedding-gte-multilingual-base` | [ ] |
