...
Srclight indexes code into a local SQLite database using hybrid search (FTS5 trigram + semantic embeddings). Embeddings are generated by the internal OpenAI-compatible inference server (`qwen3-embedding-8b`). No local model download is required.
...
| Model | Source | Action |
|-------|--------|--------|
| `qwen3-embedding-8b` | Internal inference server | Verify server is reachable: `curl http://inference.internal/v1/models` |
...
| 5 | Srclight | Correct model configured | `grep model .srclight/config.yml` shows `qwen3-embedding-8b` | [ ] |
